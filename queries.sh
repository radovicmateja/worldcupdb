...
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE game_id IS NOT NULL ORDER BY name ASC")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM games FULL JOIN teams ON teams.team_id = games.winner_id WHERE round = 'Final' ORDER BY name DESC")"
